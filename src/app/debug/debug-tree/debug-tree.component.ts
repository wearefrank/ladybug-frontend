...
export class DebugTreeComponent implements OnDestroy {
  ...
  subscribeToSubscriptions(): void {
    ...
    const refreshAll: Subscription = this.debugTab.refreshAll$.subscribe(
      (condition?: RefreshCondition) => this.refreshReports(condition),
    );
    this.subscriptions.add(refreshAll);
    const refreshTree: Subscription = this.debugTab.refreshTree$.subscribe(
      (condition?: RefreshCondition) => this.refreshReports(condition),
    );
        ...
        this.httpService
          ...
          .subscribe({
            next: (unmatched: string[]) =>
              SimpleFileTreeUtil.hideOrShowCheckpoints(
                unmatched,
                this.tree.elements.toArray(),
              ),
          });
      }
    }
  ...
  addReportToTree(report: Report): void {
    ...
    const newReport: CreateTreeItem =
      new ReportHierarchyTransformer().transform(report);
  ...
  selectAndReplaceReportIfPresent(report: Report): boolean {
    for (const index in this.tree.items) {
      ...
      if (treeReport.storageId === report.storageId) {
        const transformedReport = new ReportHierarchyTransformer().transform(
          report,
        );
        this.tree.items[index] =
          this.tree.createItemToFileItem(transformedReport);
  ...
  async refreshReports(condition?: RefreshCondition): Promise<void> {
    const selectedReportId: number =
      this.tree.getSelected().originalValue.storageId;
    let lastSelectedReport: FileTreeItem | undefined;

    const shouldProcessReport = (reportId: number) =>
    ...
    for (const index in this.tree.items) {
      ...
      if (shouldProcessReport(report.storageId)) {
        const fileItem: FileTreeItem = await this.getNewReport(
          report.storageId,
        );
  ...
  async getNewReport(storageId: number): Promise<FileTreeItem> {
    ...
    const transformedReport: Report =
      new ReportHierarchyTransformer().transform(response);
